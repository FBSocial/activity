 ...
 * @property {number} targetItem - 目标项目的索引（从1开始）
...
export default function SquaredPagerGame({
  ...
  // 将 targetItem 转换为从 0 开始的索引
  const targetIndex = targetItem - 1
  ...
  const startDraw = useCallback(() => {
    ...
    const animateDraw = (): void => {
      ...
      if (progress < 1) {
        ...
        setIsDrawing(false)
        // 在这里，onDrawComplete 回调函数将接收包含 data 字段的完整项目对象
        onDrawComplete?.(items[targetIndex])
      }
    }

    animationFrameRef.current = requestAnimationFrame(animateDraw)
  }, [isDrawing, items, targetIndex, onDrawStart, onDrawComplete, onProgress, drawDuration, calculateSwitchInterval])
    ...
    (item: SquaredPagerItem, index: number): JSX.Element => (
      ...
      >
        <div className={clsx('flex h-full w-full items-center justify-center rounded-xl', styles.prizeItem)}>
          {item.display ?
            item.image ?
              <img src={item.image} alt={item.name} className={styles.prizeImage} />
  ...
  const renderItems = useCallback(() => {
    ...
    return (
      <>
        ...
        >
          <img src={startButtonImage} alt={isDrawing ? '抽奖中' : '开始抽奖'} className="h-full w-full object-contain" />
        </div>
  ...
  return (
    <div className="squared-pager-game relative h-[22.38rem] w-[22.38rem]">
      <img src={backgroundImage} alt="游戏背景" className="absolute h-full w-full object-cover" />
