...
const DebugActionHeader: React.FC<DebugActionHeaderProps> = ({ onLogout }) => {
  ...
  return (
    <div className="fixed left-0 top-0 z-50 flex w-full items-center justify-between bg-opacity-80 bg-gradient-to-r from-blue-500 to-purple-500 p-2 px-6 text-white shadow-md">
      <div className="flex flex-1 items-center justify-start">
        <div className="mr-1 rounded-full bg-white bg-opacity-20 px-3 py-1">
        ...
        {onLogout && (
          <button onClick={onLogout} className="rounded-full bg-white bg-opacity-20 px-3 py-1 hover:bg-opacity-30">
            退出登录
          </button>
        )}
      </div>
      <div className="relative">
        <button onClick={() => setShowDetails(!showDetails)} className="rounded-full bg-white bg-opacity-20 px-3 py-1 hover:bg-opacity-30">
          更多信息
        </button>
...
const DetailsCard: React.FC<{ userToken?: string }> = () => {
  ...
  return (
    <div className="absolute right-0 mt-2 w-64 rounded-md bg-white p-4 shadow-lg">
      <h3 className="mb-2 text-lg font-semibold text-gray-800">用户信息</h3>
